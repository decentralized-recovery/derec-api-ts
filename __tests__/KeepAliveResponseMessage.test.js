...
describe("KeepAliveResponseMessage", () => {
  it("serializes", async () => {
    const keepAliveResponseMessage = new KeepAliveResponseMessage(0x1234);

    const serialized = keepAliveResponseMessage.serialize();
    expect(serialized.length).toEqual(4);
    expect(serialized[0]).toEqual(MessageType.KEEP_ALIVE_RESPONSE >> 8);
    expect(serialized[1]).toEqual(MessageType.KEEP_ALIVE_RESPONSE & 0xff);
    expect(serialized[2]).toEqual(0x12);
    expect(serialized[3]).toEqual(0x34);
