...
/**
 * Filename: PairingResponseMessage.test.js
 * Description: Tests for pairing response message
 */
import { ProtocolType, MessageType, OperatingMode } from "../../src/DeRecTypes";
import { PairingResponseMessage } from "../../src/PairingProtocol/PairingResponseMessage";
...
const testUser = {
  firstCounter: 0x12,
  nonces: new Uint8Array([3, 4, 5, 6, 7]),
...
describe("PairingResponseMessage", () => {
  it("serializes", async () => {
    const pairingResponseMessage = new PairingResponseMessage(protocolVersion, testUser.firstCounter, testUser.nonces, testUser.operatingMode, testUser.name, testUser.phone, testUser.ecdhPublicKey);
    const serialized = pairingResponseMessage.serialize(gcm, iv);
  ...
  it("deserializes", async () => {
    const pairingResponseMessage = new PairingResponseMessage();
    const deserialized = pairingResponseMessage.deserialize(gcm, iv, serializedTestMessage);
    expect(deserialized.getProtocolType()).toEqual(ProtocolType.PAIRING_PROTOCOL);
    expect(deserialized.getMessageType()).toEqual(MessageType.PAIRING_RESPONSE);
    expect(deserialized.getFirstCounter()).toEqual(testUser.firstCounter);
    expect(deserialized.getNonces()).toEqual(testUser.nonces);
  ...
  it("returns correct firstInProtocol()", async () => {
    const pairingResponseMessage = new PairingResponseMessage(protocolVersion, testUser.counter, testUser.nonce, testUser.operatingMode, testUser.name, testUser.phone, testUser.ecdhPublicKey);
    expect(pairingResponseMessage.firstInProtocol()).toEqual(false);
  });
  ...
  //   const encryptedData = PairingResponseMessage.encrypt(testData, gcm, iv);
  //   const decryptedData = PairingResponseMessage.decrypt(encryptedData, gcm, iv);
  //   expect(decryptedData).toEqual(testData);
  // });

  it("returns correct protocol version", async () => {
    const pairingResponseMessage = new PairingResponseMessage();
    expect(pairingResponseMessage.getProtocolVersion()).toEqual(protocolVersion);
  });
});
